...
══════════════════════════════════════════════════════════════ */
.main-container{position:relative;height:100vh;overflow:hidden;background:var(--w)}
...
/* ══ POURQUOI — Original hi-res image, GSAP hit zones ══════════
   Image: background-why-v2-web.jpg  2400 × 1593 px
   (downscaled from original 6967 × 4625)

   Card positions measured from 2400 × 1593 image.
   All values are top-left corner of each card as % of image.

     pays       left 20.5%  top 30.3%
     ia         left 27.5%  top 29.3%
     retention  left 34.5%  top 30.3%
     projets    left 27.5%  top 47.0%
   Card size: ~151×110 px  →  6.3% × 6.9%

   Hover animation: 100% GSAP — no CSS transition on the tile.
══════════════════════════════════════════════════════════════ */
#why-us {
  background: #fff;
  padding: 0;
...
/* Original quality photo — 2400:1593 natural ratio */
.why-bg-img {
  display: block;
  width: 100%;
  height: auto;
...
/* Fade — white table 0-40%, feathers before cards start at 45% */
.why-fade {
  position: absolute;
  inset: 0;
...
/* Copy — left 41%, well before cards at 45% */
.why-copy {
  position: absolute;
  inset: 0 59% 0 0;
  display: flex;
...
/* Mobile header above image — hidden on desktop */
.why-mobile-hd { display: none; }

/* ── Mobile layout ────────────────────────────────────────────
   Stack: [white header: title + description]
          [full image at natural ratio — no crop]
          [bullets overlaid on image left white zone]
   ─────────────────────────────────────────────────────────── */
@media (max-width: 860px) {

  /* 1. White header above image */
  .why-mobile-hd {
    display: block;
    background: #fff;
    text-align: center;
  ...
  /* 2. Image: full natural ratio, never cropped */
  .why-img-wrap {
    aspect-ratio: auto;
    overflow: hidden;
  }
  ...
  /* 3. Fade: strong white on left for bullet readability */
  .why-fade {
    background: linear-gradient(
      to right,
      rgba(255,255,255,.99) 0%,
  ...
  /* 4. Copy overlay: shifted 10% right, only bullets visible, vertically centred */
  .why-copy {
    inset: 0 38% 0 10%;
    padding: 3% 1% 3% 2%;
    justify-content: center;
  ...
  /* Hide the desktop heading + paragraph from view
     (they render in .why-mobile-hd above, aria-hidden here) */
  .why-copy > .why-h,
  .why-copy > .why-p { display: none; }

  /* 5. Accordion — 4 titles stacked, expand on tap */
  .why-pts { border-top: none; }
  .why-pt  {
  ...
  /* Trigger row — 20% less vertical padding, dark gray title */
  .why-pt-k {
    font-size: .62rem;
    font-weight: 700;
    cursor: pointer;
  ...
  .why-pt.is-open .why-pt-k::after {
    transform: rotate(45deg);
  }

  /* Description — height/opacity controlled entirely by GSAP */
  .why-pt-v {
    font-size: .56rem;
    line-height: 1.45;
    overflow: hidden;
...
.why-h em { display:block; font-style:normal; color:#09A1BE; }
.why-p {
  font-size: .85rem;
  font-weight: 300;
  color: var(--t2);
...
.why-pts { border-top: 1px solid rgba(0,0,0,.08); }
.why-pt {
  display: grid;
...
@media (max-width: 480px) { .why-pt { grid-template-columns: 1fr; gap: 4px; } }
.why-pt-k { font-size:.73rem; font-weight:700; color:#424243; }
.why-pt-v { font-size:.71rem; font-weight:300; color:var(--t2); line-height:1.62; }

/* ── Stat hit zones ─────────────────────────────────────────────
   Positions measured by pixel-level shadow detection on the
   actual 2400×1593 image (background-why-v2-web.jpg).

   Column centers (x): col1=49%  col2=58%  col3=67%
   Card half-width: 4%  → left = center - 4%
   Row 1 (stat cards): top=24%  height=13%  (y 24→37%)
   Projets (row 3):    top=51%  height=8%   (y 51→59%)

   Permanent #a100f2 glow marks zones as interactive.
   GSAP owns all hover animation — no CSS transitions on tile.
   ─────────────────────────────────────────────────────────── */
.why-stat-hit {
  position: absolute;
  z-index: 6;
  width: 9%;      /* verified from image crop: card ≈ 9% wide */
  height: 9%;     /* card ≈ 9% tall */
  min-width: 60px;
  border-radius: 10px;
  cursor: pointer;
...
/* Positions verified by cropping the actual image pixels.
   Pays: x=44-55% y=28-43% → card at y≈35%
   IA:   x=55-66% y=27-42% → card at y≈34%
   Ret:  x=66-77% y=28-43% → card at y≈35%
   Proj: x=55-66% y=47-61% → card at y≈53%        */
.why-stat-hit[data-stat="pays"]      { left: 45%;   top: 34.5%; }
.why-stat-hit[data-stat="ia"]        { left: 55.5%; top: 33.5%; }
.why-stat-hit[data-stat="retention"] { left: 66%;   top: 34.5%; }
.why-stat-hit[data-stat="projets"]   { left: 55.5%; top: 53%;   height: 9%; }

/* Black content card — shown by GSAP on hover ──────────────── */
.wsh-card {
  position: absolute;
  top: calc(100% + 10px);   /* appears below the hit zone */
  left: 50%;
  /* translate is set via GSAP so no CSS transform here */
  background: #000;
  border-radius: 10px;
  ...
  pointer-events: none;
  z-index: 10;
  /* Initial hidden state set by GSAP in JS */
}
.wsh-num {
  display: block;
...
@media (max-width: 860px) { .why-stat-hit { display: none; } }
</style>
...
<div class="main-container" id="main-container">
  ...
  <div class="slide-block" id="slide-block" data-side="left">
    ...
    <!-- <a class="slide-link" id="td-link" href="#">
      <span>Découvrir</span>
...
(function(){
  ...
  const services = [
    { idx:'01', service:'Web & Mobile',     tagline:'Développement digital',    sub:'Développement Site Web & Mobile',             side:'left',  href:'#' },
    { idx:'02', service:'Solutions IA',     tagline:'Intelligence artificielle', sub:'Solutions IA & Agents par métier et secteur', side:'right', href:'#' },
    { idx:'03', service:'SaaS & Produits',  tagline:'Plateformes robustes',     sub:'Des plateformes SaaS robustes',               side:'left',  href:'#' },
    { idx:'04', service:'Brand Experience', tagline:'Marketing digital',        sub:'Digital Marketing Agency',                    side:'right', href:'#' },
  ];
  ...
  /* currentIdx is the ground truth — updated immediately when slide changes */
  let currentIdx = 0;

  /* ── Underline pulse (CSS, no GSAP conflict) ── */
  function triggerHL(){
  ...
  /* ── Char-by-char tagline (CSS animation, no GSAP conflict) ── */
  ...
  /* ── setContent: DOM mutations ONLY — zero GSAP calls ──
     The ScrollTrigger / transitionTo() own all GSAP animation.
     Any GSAP call here on textEls would compete with transitionTo(). */
  ...
  /* ── transitionTo ────────────────────────────────────────────
     Previous bug 1: gsap.to(fade-out).onComplete → wrong svc captured
       when a second transitionTo() fired before onComplete ran.
     Previous bug 2: self.progress oscillated ±0.001 at a slide
       boundary → alternating idx triggered rapid back-to-back calls.

     Fix 1: no onComplete chain. Instantly hide with gsap.set,
       swap content while hidden, immediately start fade-in.
       overwrite:true cancels any competing fade-in tween.
     Fix 2: pendingIdx / pendingCount require the new idx to be
       stable for 2 consecutive onUpdate frames before committing.
       Eliminates boundary flicker without slowing real transitions.
  ── */
  let pendingIdx   = -1;
  let pendingCount = 0;
  ...
  function transitionTo(idx){
    ...
    /* Instantly hide — zero-duration, no onComplete, no race */
    gsap.set(textEls, { opacity:0 });

    /* Swap DOM content while text is invisible */
    setContent(svc);

    /* Block snaps to new position (text is hidden so no visible jump) */
    gsap.set(elBlock, { left: POS[svc.side] });

    /* Fade in — overwrite:true cancels any leftover tween */
    gsap.fromTo(textEls,
      { opacity:0, y:10 },
  ...
  /* ── Single ScrollTrigger: pin + scrub for video, onUpdate for slides ── */
  ScrollTrigger.create({
    ...
    pin:     true,
    anticipatePin: 1,
    /* Tighter scrub on touch devices → transitions feel instant not laggy */
    scrub: window.matchMedia('(pointer:coarse)').matches ? 0.2 : 0.6,
    onUpdate(self){
      /* Video: scrub ends at 4.5 s — eyes fully open */
      if(mainVid.readyState >= 2 && mainVid.duration)
        mainVid.currentTime = clamp(0, 4.5, self.progress * 4.5);

      /* Slide index from progress — correct in both directions */
      const idx = Math.min(services.length - 1, Math.floor(self.progress * services.length));

      dots.forEach((d, i) => d.classList.toggle('active', i === idx));

      if(idx !== currentIdx){
        /* Require 2 stable frames at the new idx before committing.
           Prevents a single-frame flicker at slide boundaries from
           triggering a transition that immediately reverses. */
        if(idx === pendingIdx){
          pendingCount++;
          if(pendingCount >= 2){
            pendingIdx   = -1;
        ...
        /* Reset pending when we're back at the current service */
        pendingIdx   = -1;
        pendingCount = 0;
      }
    }

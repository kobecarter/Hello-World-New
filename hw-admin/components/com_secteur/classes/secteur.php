...
class secteur
{
    ...
    public function add()
    {
        ...
        if (!$db->query($SQLinsert)) 
        {
            ...
            $SQLinsert2 = sprintf("INSERT INTO " . static::$table2 . " (id_secteur, titre, h1, slug, sous_titre, extrait, texte, seo_titre, seo_description, seo_keyword, langue) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    public function edit()
    {
        ...
        if (!$db->query($SQLupdate)) {
            ...
            if ($db->num_rows($result) == 0) {
                $SQLupdate = sprintf("INSERT INTO " . static::$table2 . " (id_secteur, titre, h1, slug, sous_titre, extrait, texte, seo_titre, seo_description, seo_keyword, langue) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    GetSQLValueString($this->id, "int"),
                    GetSQLValueString($this->titre, "text"),
                    GetSQLValueString($this->h1, "text"),
                    GetSQLValueString($this->slug, "text"),
                ...
                $SQLupdate = sprintf("UPDATE " . static::$table2 . " SET titre = %s, h1 = %s, slug = %s, sous_titre = %s, extrait = %s, texte = %s, seo_titre = %s, seo_description = %s, seo_keyword = %s WHERE id_secteur = %s AND langue = %s",
                    GetSQLValueString($this->titre, "text"),
                    GetSQLValueString($this->h1, "text"),
                    GetSQLValueString($this->slug, "text"),
                    GetSQLValueString($this->sous_titre, "text"),
...
public static function findAll($langue, $active = false, $services = false, $currentPage = false, $limit = false)
{
   ...
   if ($services !== false) {
    ...
    public static function build($data){
        ...
        $secteur->setSeoKeyword(isset($data['seo_keyword']) ? $data['seo_keyword'] : null);

...
<div class="main-content">
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des secteurs</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    <caption style="text-align: left; margin-left: 7px; margin-top: 20px;">
                        ...
                        <?php if($_SESSION['user']->hasDroit('edit', 'com_secteur')){ ?>
                        ...
                        <?php if($_SESSION['user']->hasDroit('delete', 'com_secteur')){ ?>
                    ...
                    <?php foreach ($secteurs as $secteur): ?>
                        <tr id="row_<?= $secteur->getId(); ?>">
                            <td>
                                <input type="checkbox" class="checkElement" value="<?php echo $secteur->getId();?>">
                            </td>
                            <td><?= $secteur->getId(); ?></td>
                            <td><?= $secteur->getTitre(); ?></td>
                            <td><?= $secteur->getSousTitre(); ?></td>
                            <td class="text-center">
                                <?php if ($secteur->isActive() && $_SESSION['user']->hasDroit('edit', 'com_secteur')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $secteur->getId(); ?>_oui"
                                ...
                                <?php } else if (!$secteur->isActive() && $_SESSION['user']->hasDroit('edit', 'com_secteur')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $secteur->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_secteur')) { ?>
                                    <a href="index.php?option=com_secteur&task=edit&id=<?= $secteur->getId(); ?>"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_secteur')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $secteur->getId(); ?>"
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer cet secteur ?")) {
                var btn = $(this);
                var t = btn.attr("id").split("_");
                var id = t[1];
                var order = "id=" + id;
                $.secteur("components/com_secteur/controleurs/router.php?task=deleteSecteur", order, function (theResponse) {
                    var success_msg = "Secteur supprimé avec succès.";
        ...
        $(".enable").click(function () {
            ...
            $.secteur("components/com_secteur/controleurs/router.php?task=enableSecteur", order, function (theResponse) {
    ...
    $(document).ready(function()
    {
        ...
        $('#enable_multiple, #disable_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous " + activationTxt + " les secteurs sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids + '&active=' + active ;
                    $.secteur(
                        "components/com_secteur/controleurs/router.php?task=enableSecteurs", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1){
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les secteurs sont activ&eacute;s avec succ&egrave;s</div>');	
                                $('.msgbox').slideDown();
        ...
        $('#delete_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous supprimer les secteurs sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids;
                    $.secteur(
                        "components/com_secteur/controleurs/router.php?task=deleteSecteurs", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)
                            {
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les secteurs ont été supprimés avec succès</div>');	
                                $('.msgbox').slideDown();

...
function deleteAgentIa($data)
{
    ...
    if (validateAgentIa($data, $indices)) {
        ...
        if ($agent->delete() == 1) {
            ...
            if ($agent->getPhotoHero() && file_exists("../../../../images/agents_ia/" . $agent->getPhotoHero())) {
                @unlink("../../../../images/agents_ia/" . $agent->getPhotoHero());
            }
            if ($agent->getPhotoFullBody() && file_exists("../../../../images/agents_ia/" . $agent->getPhotoFullBody())) {
                @unlink("../../../../images/agents_ia/" . $agent->getPhotoFullBody());
            }
            if ($agent->getPhotoProduit() && file_exists("../../../../images/agents_ia/" . $agent->getPhotoProduit())) {
                @unlink("../../../../images/agents_ia/" . $agent->getPhotoProduit());
            }
            echo "1";
        } else {
            echo "2";
...
function buildAgentIa($data, $id = null)
{
    ...
    if (isset($_FILES['photo_hero']) && $_FILES['photo_hero']['name'][0] != '') {
        $photo_hero = uploadFiles('photo_hero', $imgDir, $allowedExt);
    }
    if (isset($_FILES['photo_full_body']) && $_FILES['photo_full_body']['name'][0] != '') {
        $photo_full_body = uploadFiles('photo_full_body', $imgDir, $allowedExt);
    }
    if (isset($_FILES['photo_produit']) && $_FILES['photo_produit']['name'][0] != '') {
        $photo_produit = uploadFiles('photo_produit', $imgDir, $allowedExt);
    }
    ...
    if ($id) {
        ...
        if (isset($photo_hero[0])) {
            $agent->setPhotoHero($photo_hero[0]);
            if ($existing->getPhotoHero() && file_exists($imgDir . $existing->getPhotoHero())) {
                @unlink($imgDir . $existing->getPhotoHero());
            }
        } else {
            $agent->setPhotoHero($existing->getPhotoHero());
        }

        if (isset($photo_full_body[0])) {
            $agent->setPhotoFullBody($photo_full_body[0]);
            if ($existing->getPhotoFullBody() && file_exists($imgDir . $existing->getPhotoFullBody())) {
                @unlink($imgDir . $existing->getPhotoFullBody());
            }
        } else {
            $agent->setPhotoFullBody($existing->getPhotoFullBody());
        }

        if (isset($photo_produit[0])) {
            $agent->setPhotoProduit($photo_produit[0]);
            if ($existing->getPhotoProduit() && file_exists($imgDir . $existing->getPhotoProduit())) {
                @unlink($imgDir . $existing->getPhotoProduit());
            }
        } else {
            $agent->setPhotoProduit($existing->getPhotoProduit());
        }
        ...
        $agent->setPhotoHero(isset($photo_hero[0]) ? $photo_hero[0] : null);
        $agent->setPhotoFullBody(isset($photo_full_body[0]) ? $photo_full_body[0] : null);
        $agent->setPhotoProduit(isset($photo_produit[0]) ? $photo_produit[0] : null);
        $slug = agent_ia::generateSlug(
            (isset($data['slug']) && !empty($data['slug']) ? $data['slug'] : $data['titre']),
            $_SESSION['langue']
        );

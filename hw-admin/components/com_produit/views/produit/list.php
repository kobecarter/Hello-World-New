...
<div class="main-content">
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des produits</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    <caption style="text-align: left; margin-left: 7px; margin-top: 20px;">
                        ...
                        <?php if($_SESSION['user']->hasDroit('edit', 'com_produit')){ ?>
                        ...
                        <?php if($_SESSION['user']->hasDroit('delete', 'com_produit')){ ?>
                    ...
                    <tr>
                        ...
                        <th>
                            <?php
                                if(isset($trad_com_produit['PRIX'][$_SESSION['user']->getLangue()]))
                                    echo $trad_com_produit['PRIX'][$_SESSION['user']->getLangue()];
                                else
                                    echo "Prix";
                            ?>
                        </th>
                        <th>
                            <?php
                                if(isset($trad_com_produit['CAT'][$_SESSION['user']->getLangue()]))
                                    echo $trad_com_produit['CAT'][$_SESSION['user']->getLangue()];
                                else
                                    echo "Categorie";
                            ?>
                        </th>
                    ...
                    <?php foreach ($produits as $produit): ?>
                        <tr id="row_<?= $produit->getId(); ?>">
                            <td>
                                <input type="checkbox" class="checkElement" value="<?php echo $produit->getId();?>">
                            </td>
                            <td><?= $produit->getId(); ?></td>
                            <td><?= $produit->getTitre(); ?></td>
                            <td><?= $produit->getPrix(); ?> <b><?=$produit->getDevise()?></b></td>
                            <td><?= $produit->getCategorie()->getTitre(); ?></td>
                            <td class="text-center">
                                <?php if ($produit->isActive() && $_SESSION['user']->hasDroit('edit', 'com_produit')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $produit->getId(); ?>_oui"
                                ...
                                <?php } else if (!$produit->isActive() && $_SESSION['user']->hasDroit('edit', 'com_produit')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $produit->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_produit')) { ?>
                                    <a href="index.php?option=com_produit&task=edit&id=<?= $produit->getId(); ?>"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_produit')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $produit->getId(); ?>"
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer cet produit ?")) {
                var btn = $(this);
                ...
                $.post("components/com_produit/controleurs/router.php?task=deleteProduit", order, function (theResponse) {
                    var success_msg = "Article supprimé avec succès.";

...
class agent_ia
{
    ...
    public function add()
    {
        global $db;
        $SQLinsert = sprintf(
            "INSERT INTO " . static::$table . " (photo, photo_banniere, active, date_add, last_edit) VALUES (%s, %s, %s, %s, %s)",
    ...
    public function edit()
    {
        global $db;
        $SQLupdate = sprintf(
            "UPDATE " . static::$table . " SET photo = %s, photo_banniere = %s, active = %s, last_edit = %s WHERE id = %s",
            GetSQLValueString($this->photo, "text"),
            GetSQLValueString($this->photo_banniere, "text"),
            GetSQLValueString($this->active, "int"),
            GetSQLValueString($this->last_edit, "date"),
    ...
    public static function findAll($langue, $active = false, $currentPage = false, $limit = false)
    {
        ...
        $items = array();
        $SQLselect = sprintf(
            "SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A INNER JOIN " . static::$table2 . " B ON A.id = B.id_agent_ia WHERE B.langue = %s",
            GetSQLValueString($langue, "text")
        );
        if ($active) {

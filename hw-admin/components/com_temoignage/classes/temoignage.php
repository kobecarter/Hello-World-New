...
class temoignage
{
    ...
    public function add()
    {
        ...
        $SQLinsert = sprintf("INSERT INTO " . static::$table . " (photo, active, global, ordre, date_add, last_edit) VALUES (%s, %s, %s, %s, %s, %s)",
            GetSQLValueString($this->photo, "text"),
            GetSQLValueString($this->active, "int"),
            GetSQLValueString($this->global, "int"),
        ...
        if (!$db->query($SQLinsert)) {
            ...
            $SQLinsert2 = sprintf("INSERT INTO " . static::$table2 . " (id_temoignage, nom, fonction, email, sujet, temoignage, langue) VALUES (%s, %s, %s, %s, %s, %s, %s)",
    ...
    {
        global $db;
        $SQLupdate = sprintf("UPDATE " . static::$table . " SET  photo = %s, active = %s,  global = %s, ordre = %s, last_edit = %s WHERE id = %s",
            GetSQLValueString($this->photo, "text"),
            GetSQLValueString($this->active, "int"),
            GetSQLValueString($this->global, "int"),
            GetSQLValueString($this->ordre, "int"),
        ...
        if (!$db->query($SQLupdate)) {
            $SQLselect = sprintf("SELECT * FROM " . static::$table2 . " WHERE id_temoignage = %s AND langue = %s",
                GetSQLValueString($this->id, "int"),
                GetSQLValueString($this->langue, "text")
            );
            $result = $db->query($SQLselect);
            if ($db->num_rows($result) == 0) {
                $SQLupdate = sprintf("INSERT INTO " . static::$table2 . " (id_temoignage, nom, fonction, email, sujet, temoignage, langue) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    GetSQLValueString($this->id, "int"),
                    GetSQLValueString($this->nom, "text"),
                    GetSQLValueString($this->fonction, "text"),
                    GetSQLValueString($this->email, "text"),
                ...
                $SQLupdate = sprintf("UPDATE " . static::$table2 . " SET nom = %s, fonction = %s, email = %s, sujet = %s, temoignage = %s WHERE id_temoignage = %s AND langue = %s",
                    GetSQLValueString($this->nom, "text"),
                    GetSQLValueString($this->fonction, "text"),
                    GetSQLValueString($this->email, "text"),
                    GetSQLValueString($this->sujet, "text"),
    ...
    public function delete()
    {
        ...
        $SQLdelete2 = sprintf("DELETE FROM " . static::$table2 . " WHERE id_temoignage = %s",
            GetSQLValueString($this->id, "int")
        );
    ...
    public static function find($id, $langue)
    {
        ...
        $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_temoignage AND langue = %s WHERE A.id = %s",
    ...
    public static function findAll($langue, $active = false, $ordre = false)
    {
        global $db;
        $items = array();
        $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_temoignage AND langue = %s WHERE 1 = 1",
            GetSQLValueString($langue, "text")
        );
    ...
    public static function findAllGlobal($langue, $active = false, $global = false, $ordre = false)
    {
        global $db;
        $items = array();
        $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_temoignage AND langue = %s WHERE 1 = 1",
    ...
    public static function build($data){
        ...
        $temoignage->setActive($data['active']);
          $temoignage->setGlobal($data['global']);

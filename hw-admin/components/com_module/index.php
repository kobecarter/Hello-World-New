    ...
    function showList()
    {
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                var warning = "<?= $trad_com_module['ATTENTION'][$_SESSION['user']->getLangue()];?>";
                ...
                $(".install").click(function (event) {
                    ...
                    var warning_msg = "<?= $trad_com_module['WARNING_INSTALL'][$_SESSION['user']->getLangue()];?>";
                    var t = $(this).attr("id").split("-");
                    var id = t[1];
                    var order = 'id=' + id;
                    $.post("components/com_module/controleur/module.php?task=installModule", order, function (theResponse) {
                        if (parseInt(theResponse) === 1) {
                            ...
                            $('.msgbox').html('<div class="alert alert-warning alert-dismissable"><i class="fa fa-exclamation-triangle"></i> <strong>' + warning + '</strong> ' + warning_msg + '</div>').slideDown();
                        }
                        else {
                            $('.msgbox').html('<div class="alert alert-danger alert-dismissable"><i class="icon-remove-sign"></i> <strong>' + error + '</strong> ' + error_msg + '</div>').slideDown();
                        }

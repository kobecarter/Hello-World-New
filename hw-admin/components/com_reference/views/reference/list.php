...
<div class="main-content">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Liste des références</li>
    </ol>
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des références</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    ...
                    <?php foreach ($references as $reference): ?>
                        <tr id="row_<?= $reference->getId(); ?>">
                            <td>
                                <input type="checkbox" class="checkElement" value="<?php echo $reference->getId();?>" >
                            </td>
                            <td><?= $reference->getId(); ?></td>
                            <td><?= $reference->getNomClient(); ?></td>
                            <td><?= $reference->getSiteWeb(); ?></td>
                            <td class="text-center">
                                <?php if ($reference->isActive() && $_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $reference->getId(); ?>_oui"
                                ...
                                <?php } else if (!$reference->isActive() && $_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $reference->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                    <a href="index.php?option=com_reference&task=addItem&id=<?php echo $reference->getId(); ?>"
                                        data-toggle="tooltip" data-placement="top"
                                        data-original-title="<?= isset($trad_com_reference['GERER_ELEMETS'][$_SESSION['user']->getLangue()]) ? $trad_com_reference['GERER_ELEMETS'][$_SESSION['user']->getLangue()] : "Gérer les éléments" ;?>" 
                                        class="btn btn-default btn-xs"><i class="icon-reorder"></i></a>
                                <?php } ?>
								<?php if ($_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                    <a href="index.php?option=com_reference&task=addCursus&id=<?php echo $reference->getId(); ?>"
                                        data-toggle="tooltip" data-placement="top"
                                        data-original-title="Gérer les cursus" 
                                        class="btn btn-info btn-xs"><i class="fa fa-list-ol"></i></a>
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                    <a href="index.php?option=com_reference&task=edit&id=<?= $reference->getId(); ?>"
                                       data-toggle="tooltip" data-placement="top" 
                                       data-original-title="<?= isset($trad_com_reference['MODIFIER'][$_SESSION['user']->getLangue()]) ? $trad_com_reference['MODIFIER'][$_SESSION['user']->getLangue()] : "Modifier" ;?>" 
                                       class="btn btn-warning btn-xs"><i class="fa fa-pencil-alt icon-pencil"></i></a>
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_reference')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $reference->getId(); ?>"
                                        data-toggle="tooltip" data-placement="top" <?= isset($trad_com_reference['MODIFIER'][$_SESSION['user']->getLangue()]) ? $trad_com_reference['MODIFIER'][$_SESSION['user']->getLangue()] : "Modifier" ;?>" 
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer ce reference ?")) {
                var btn = $(this);
                var t = btn.attr("id").split("_");
                var id = t[1];
                var order = "id=" + id;
                $.post("components/com_reference/controleurs/router.php?task=deleteReference", order, function (theResponse) {
        ...
        $(".enable").click(function () {
            ...
            $.post("components/com_reference/controleurs/router.php?task=enableReference", order, function (theResponse) {
    ...
    $(document).ready(function()
    {
        ...
        $('#enable_multiple, #disable_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous " + activationTxt + " les references sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids + '&active=' + active ;
                    $.post(
                        "components/com_reference/controleurs/router.php?task=enableReferences", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1){
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les references est activ&eacute;s avec succ&egrave;s</div>');	
                                $('.msgbox').slideDown();
        ...
        $('#delete_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous supprimer les references sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids;
                    $.post(
                        "components/com_reference/controleurs/router.php?task=deleteReferences", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)

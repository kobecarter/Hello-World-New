...
<div class="widget widget-green">
    <div class="widget-title">
        ...
        <h3><i class="icon-table"></i> Liste des cursus</h3>
    </div>
    <div class="widget-content">
        <div class="table-responsive">
            ...
            <?php
                ...
                {
            ?>
                <input type="submit" value="Ajouter cursus" id='ajouter' class="btn btn-primary submit"
            ...
            <table class="table table-bordered table-hover datatable">
                ...
                <tbody>
                <?php foreach ($cursuss as $cursus): ?>
                    <tr id="row_<?= $cursus->getId(); ?>">
                        <td>
                            <input type="checkbox" class="checkElement" value="<?php echo $cursus->getId();?>" >
                        </td>
                        <td><?= $cursus->getId(); ?></td>
                        <td><?= $cursus->getTitre(); ?></td>
                        <td><?= $cursus->getOrdre(); ?></td>
                        <td class="text-center">
                            <?php if ($_SESSION['user']->hasDroit('edit', 'com_reference')) { ?>
                                <a href="index.php?option=com_reference&task=editCursus<?= "&id=".$_GET['id']."&id_cursus=" . $cursus->getId(); ?>"
                            ...
                            <?php } ?>
                            <?php if ($_SESSION['user']->hasDroit('delete', 'com_reference')) { ?>
                                <a href="javascript:void(0)" id="delete_<?= $cursus->getId(); ?>"
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer ce cursus ?")) {
                var btn = $(this);
                var t = btn.attr("id").split("_");
                var id = t[1];
                var order = "id=" + id;
                $.post("components/com_reference/controleurs/router.php?task=deleteCursus", order, function (theResponse) {
                    var success_msg = "Cursus supprimé avec succès.";
    ...
    $(document).ready(function()
    {
        ...
        {
            location = "index.php?option=com_reference&task=addCursus&id=<?= $_GET['id'] ?>";
        });
        ...
        $('#delete_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous supprimer les cursus sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids;
                    $.post(
                        "components/com_reference/controleurs/router.php?task=deleteCursuss", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)
                            {
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les cursus ont été supprim&eacute;s avec succ&egrave;s</div>');	
                                $('.msgbox').slideDown();

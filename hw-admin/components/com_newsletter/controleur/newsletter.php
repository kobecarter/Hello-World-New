    ...
    if(isset($_FILES['fichier']) && $_FILES['fichier']['name'][0]!='') {
        ...
        if(@move_uploaded_file($file_nom_tmp, $file)){
            ...
            foreach($data as $value){
                $insertSQL = sprintf("INSERT INTO ".__prefixe_db__."newsletter (nom, email, date_add, confirm) VALUES (%s, %s, %s, %s)",
...
function deleteNewsletter($data){
    ...
    if(isset($data['id']) && !empty($data['id'])){
        $id = intval($data['id']);
        $SQLdelete = "DELETE FROM ".__prefixe_db__."newsletter WHERE id = $id";

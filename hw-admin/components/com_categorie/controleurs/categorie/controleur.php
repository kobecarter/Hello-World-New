...
function buildCategorie($data, $id = null)
{
    ...
    if($id){
        ...
        $slug = categorie::generateSlug((isset($data['slug']) && !empty($data['slug']) ? $data['slug'] : $data['titre']),$_SESSION['langue'],$id);
        $data['slug'] = $slug;
    } else {
        if(isset($photo[0]) ) {
            $categorie->setPhoto($photo[0]);
        ...
        $slug = categorie::generateSlug((isset($data['slug']) && !empty($data['slug']) ? $data['slug'] : $data['titre']),$_SESSION['langue']);
        $data['slug'] = $slug;
    }

    $categorie->setParent(categorie::find($data["id_parent"], $_SESSION["langue"]));

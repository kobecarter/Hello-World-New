<div class="sub-sidebar-wrapper">
    <ul>
        <?php if ($_SESSION['user']->hasDroit('add', 'com_job')) { ?>
            <li><a href="index.php?option=com_job&task=add"> Ajouter Offre de travail</a></li>
        <?php } ?>
        <?php if ($_SESSION['user']->hasDroit('view', 'com_job')) { ?>
            <li><a href="index.php?option=com_job"> Liste des offres de travail</a></li>
        <?php } ?>
    </ul>
</div>
...
<div class="main-content">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Liste des ofres de travail</li>
    </ol>
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des offres de travail</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    <caption style="text-align: left; margin-left: 7px; margin-top: 20px;">
                        ...
                        <?php if($_SESSION['user']->hasDroit('edit', 'com_job')){ ?>
                        ...
                        <?php if($_SESSION['user']->hasDroit('delete', 'com_job')){ ?>
                    ...
                    <?php foreach ($jobs as $job): ?>
                        <tr id="row_<?= $job->getId(); ?>">
                            <td>
                                <input type="checkbox" class="checkElement" value="<?php echo $job->getId();?>" style="/*padding: 10px; margin-right: 20px;*/">
                            </td>
                            <td><?= $job->getId(); ?></td>
                            <td><?= $job->getTitre(); ?></td>
                            <td><?= $job->getOrdre(); ?></td>
                            <td class="text-center">
                                <?php if ($job->isActive() && $_SESSION['user']->hasDroit('edit', 'com_job')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $job->getId(); ?>_oui"
                                ...
                                <?php } else if (!$job->isActive() && $_SESSION['user']->hasDroit('edit', 'com_job')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $job->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_job')) { ?>
                                    <a href="index.php?option=com_job&task=edit&id=<?= $job->getId(); ?>"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_job')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $job->getId(); ?>"
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer cet offre ?")) {
                ...
                var id = t[1];
                var order = "id=" + id;
                $.post("components/com_job/controleurs/router.php?task=deleteJob", order, function (theResponse) {
        ...
        $(".enable").click(function () {
            ...
            $.post("components/com_job/controleurs/router.php?task=enableJob", order, function (theResponse) {
    ...
    $(document).ready(function()
    {
        ...
        $('#enable_multiple, #disable_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous " + activationTxt + " les offres sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids + '&active=' + active ;
                    $.post(
                        "components/com_job/controleurs/router.php?task=enableJobs", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1){
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les offres est activ&eacute;s avec succ&egrave;s</div>');	
                                $('.msgbox').slideDown();
        ...
        $('#delete_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous supprimer les offres sélectionnés ??"))
                {
                    var ids = "(" + idsT + ")";

                    var order = 'ids=' + ids;
                    $.post(
                        "components/com_job/controleurs/router.php?task=deleteJobs", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)
                            {
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les offres supprim&eacute;s avec succ&egrave;s</div>');	
                                $('.msgbox').slideDown();

...
class blog
{
    ...
    public function add()
    {
        ...
        $SQLinsert = sprintf("INSERT INTO " . static::$table . " (id_categorie, photo, photo_banniere, active, date_add, last_edit) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    {
        global $db;
        $SQLupdate = sprintf("UPDATE " . static::$table . " SET id_categorie = %s, photo = %s, photo_banniere = %s, active = %s, last_edit = %s WHERE id = %s",
            GetSQLValueString($this->categorie->getId(), "int"),
            GetSQLValueString($this->photo, "text"),
            GetSQLValueString($this->photo_banniere, "text"),
            GetSQLValueString($this->active, "int"),
        ...
        if (!$db->query($SQLupdate)) {
            $SQLselect = sprintf("SELECT * FROM " . static::$table2 . " WHERE id_blog = %s AND langue = %s",
                GetSQLValueString($this->id, "int"),
                GetSQLValueString($this->langue, "text")
            );
    ...
    public function delete()
    {
        ...
        $SQLdelete2 = sprintf("DELETE FROM " . static::$table2 . " WHERE id_blog = %s",
            GetSQLValueString($this->getId(), "int")
        );
    ...
    public function getCategorieLink(){
        global $siteURL;
        if ($this->getCategorie()->getSlug() != "") {
            if (langue::isLangueDefault($this->langue)) {
                return $siteURL . "blog-categorie/" . $this->getCategorie()->getSlug() . "/";
            } else {
                return $siteURL . $this->getLangue() . "/blog-categorie/" . $this->getCategorie()->getSlug() . "/";
            }
        } else
            return "index.php?option=com_" . __CLASS__ . "&task=categorie&l=".$this->langue."&slug=".$this->getCategorie()->getSlug();
    }
    ...
    public static function getSeo(){
        ...
        RewriteRule ^blog-categorie/([^/]+)/$ index.php?option=com_" . __CLASS__ . "&task=categorie&slug=$1&l=fr [NC,L]
        RewriteRule ^([a-z]+)/blog-categorie/([^/]+)/$ index.php?option=com_" . __CLASS__ . "&task=categorie&l=$1&slug=$2 [NC,L]
        ";
        return $url;
    }
    ...
    public static function find($id, $langue)
    {
        global $db;
        $blog = new blog();
        $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_blog AND langue = %s WHERE A.id = %s",
    ...
    public static function findBySlug($slug, $langue)
    {
        global $db;
        $blog = new blog();
        $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_blog AND langue = %s WHERE B.slug = %s",
            GetSQLValueString($langue, "text"),
    ...
    public static function generateSlug($slug, $langue , $id = false)
{
    ...
    do {
        $SQLselect = sprintf(
            "SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A 
             LEFT JOIN " . static::$table2 . " B ON A.id = B.id_blog AND langue = %s 
             WHERE slug = %s",
            GetSQLValueString($langue, "text"),
    ...
    //     $newSlug = url_rewriting($slug);
    //     do{
    //         $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_blog AND langue = %s WHERE slug = %s",
    //             GetSQLValueString($langue, "text"),
    //             GetSQLValueString($newSlug, "text")
    //         );
    //         if($id){
...
//         global $db;
//         $items = array();
//         $SQLselect = sprintf("SELECT A.id as ID, A.*, B.* FROM " . static::$table . " A LEFT JOIN " . static::$table2 . " B ON A.id = B.id_blog AND langue = %s WHERE 1 = 1",
//             GetSQLValueString($langue, "text")
//         );
//         if($active){
...
public static function findAll($langue, $active = false, $categories = false, $currentPage = false, $limit = false)
{
    global $db;
    $items = array();
    ...
    $SQLselect = sprintf(
        "SELECT A.id as ID, A.*, B.*
         FROM " . static::$table . " A
         INNER JOIN " . static::$table2 . " B ON A.id = B.id_blog
         WHERE B.langue = %s",
        GetSQLValueString($langue, "text")
    );
   ...
   if ($categories !== false) {
        if (is_array($categories)) {
            ...
            $SQLselect .= " AND A.id_categorie IN (" . implode(',', $categories) . ")";
        } else {
            // Si une seule catégorie
            $SQLselect .= " AND A.id_categorie = " . intval($categories);
        }
    }
    ...
    return $items;
      ...
      public static function findSimilar($categorie, $active, $langue, $limit = false)
    {
        global $db;
        $items = array();
        $SQLselect = sprintf(
    ...
    public static function build($data){
        ...
        $blog->setId($data['ID']);
        $blog->setCategorie(categorie::find($data['id_categorie'],$data['langue']));

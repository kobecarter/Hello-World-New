...
<div class="main-content">
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des faqs</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    <caption style="text-align: left; margin-left: 7px; margin-top: 20px;">
                        ...
                        <?php if($_SESSION['user']->hasDroit('edit', 'com_faq')){ ?>
                        ...
                        <?php if($_SESSION['user']->hasDroit('delete', 'com_faq')){ ?>
                        ...
                        <tr id="row_<?= $faq->getId(); ?>">
                            ...
                            <td class="text-center">
                                <?php if ($faq->isActive() && $_SESSION['user']->hasDroit('edit', 'com_faq')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $faq->getId(); ?>_oui"
                                ...
                                <?php } else if (!$faq->isActive() && $_SESSION['user']->hasDroit('edit', 'com_faq')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $faq->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_faq')) { ?>
                                    <a href="index.php?option=com_faq&task=edit&id=<?= $faq->getId(); ?>"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_faq')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $faq->getId(); ?>"
...
<script type="text/javascript">
    $(function () {
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer cet faq ?")) {
                var btn = $(this);
                var t = btn.attr("id").split("_");
                var id = t[1];
                var order = "id=" + id;
                $.post("components/com_faq/controleurs/router.php?task=deleteFaq", order, function (theResponse) {
                    var success_msg = "Faq supprimé avec succès.";
        ...
        $(".enable").click(function () {
            ...
            $.post("components/com_faq/controleurs/router.php?task=enableFaq", order, function (theResponse) {
        ...
        $('#enable_multiple, #disable_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous " + activationTxt + " les articles sélectionnés ??"))
                {
                    ...
                    $.post(
                        "components/com_faq/controleurs/router.php?task=enableFaqs", 
                        order, 
        ...
        $('#delete_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous supprimer les articles sélectionnés ??"))
                {
                    ...
                    var order = 'ids=' + ids;
                    $.post(
                        "components/com_faq/controleurs/router.php?task=deleteFaqs", 
                        order, 
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)

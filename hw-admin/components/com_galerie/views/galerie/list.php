<div class="sub-sidebar-wrapper">
    <ul>
        <?php if ($_SESSION['user']->hasDroit('add', 'com_galerie')) { ?>
            <li><a href="index.php?option=com_galerie&task=add"> Ajouter galerie</a></li>
        <?php } ?>
...
<div class="main-content">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Liste des galeries</li>
    </ol>
    ...
    <div class="widget widget-blue">
        <div class="widget-title">
            ...
            <h3><i class="icon-table"></i> Liste des galeries</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover datatable">
                    <caption style="text-align: left; margin-left: 7px; margin-top: 20px;">
                        ...
                        <?php if($_SESSION['user']->hasDroit('edit', 'com_galerie')){ ?>
                        ...
                        <?php if($_SESSION['user']->hasDroit('delete', 'com_galerie')){ ?>
                    ...
                    <tr>
                        ...
                        <th>
                            <?php
                                if(isset($trad_com_galerie['ACTION'][$_SESSION['user']->getLangue()]))
                                    echo $trad_com_galerie['ACTION'][$_SESSION['user']->getLangue()];
                                else
                                    echo "Action";
                            ?>
                        </th>
                    ...
                    <?php foreach ($galeries as $galerie): ?>
                        <tr id="row_<?= $galerie->getId(); ?>">
                            ...
                            <td class="text-center">
                                <?php if ($galerie->isActive() && $_SESSION['user']->hasDroit('edit', 'com_galerie')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $galerie->getId(); ?>_oui"
                                ...
                                <?php } else if (!$galerie->isActive() && $_SESSION['user']->hasDroit('edit', 'com_galerie')) { ?>
                                    <a href="javascript:void(0)" id="enable_<?= $galerie->getId(); ?>_non"
                                ...
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_galerie')) { ?>
                                    <a href="index.php?option=com_galerie&task=addPhoto&id=<?php echo $galerie->getId(); ?>"
                                        data-toggle="tooltip" data-placement="top"
                                        data-original-title="Gérer les photos" 
                                        class="btn btn-default btn-xs"><i class="icon-reorder"></i></a>
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('edit', 'com_galerie')) { ?>
                                    <a href="index.php?option=com_galerie&task=edit&id=<?= $galerie->getId(); ?>"
                                       data-toggle="tooltip" data-placement="top" 
                                       data-original-title="Modifier" 
                                       class="btn btn-warning btn-xs"><i class="fa fa-pencil-alt icon-pencil"></i></a>
                                <?php } ?>
                                <?php if ($_SESSION['user']->hasDroit('delete', 'com_galerie')) { ?>
                                    <a href="javascript:void(0)" id="delete_<?= $galerie->getId(); ?>"
...
<script type="text/javascript">
        ...
        $(".delete").click(function () {
            if (confirm("Voulez vous supprimer cette galerie ?")) {
                ...
                $.post("components/com_galerie/controleurs/router.php?task=deleteGalerie", order, function (theResponse) {
                    var success_msg = "Galerie supprimée avec succès.";
    ...
    $(document).ready(function()
    {
        ...
        $('#enable_multiple, #disable_multiple').click(function()
        {
            ...
            if(test)
            {
                if(confirm("Voulez vous " + activationTxt + " les galeries sélectionnées ??"))
                {
                    var ids = "(" + idsT + ")";
                    ...
                    $.post(
                        ...
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1){
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les galeries sont activées avec succès</div>');	
                                $('.msgbox').slideDown();
                    ...
                    $.post(
                        ...
                        function(theResponse)
                        {
                            if(parseInt(theResponse) == 1)
                            {
                                ...
                                $('.msgbox').html('<div class="alert alert-success alert-dismissable"><i class="icon-check-sign"></i> <strong>Succès</strong> les galeries ont été supprimées avec succès</div>');	
                                $('.msgbox').slideDown();

(function ($) {
  ...
  */
  ...
  /*--------------------------------------------------------------
    5. Slick Slider
  --------------------------------------------------------------*/
function fixFocusableInHiddenSlides($slider) {
  $slider.find('.slick-slide').each(function() {
    var $slide = $(this);
    if ($slide.attr('aria-hidden') === 'true') {
      // Slide masquée → rendre tous les éléments interactifs non-focusables
      $slide.find('a, button, input, textarea, select').attr('tabindex', '-1');
    } else {
      // Slide visible → rendre les éléments focusables normalement
  ...
  function slickInit() {
    ...
    $('.slider-for, .slider-nav, slider-for_item').on('init afterChange', function() {
  fixFocusableInHiddenSlides($(this));
});

// Déclencher l’update une fois après l’init
$('.slider-for, .slider-nav, slider-for_item').slick('setPosition');
  }
  ...
  $(document).on('click', '.cs-video_open', function (e) {
    e.preventDefault();
    // prendre l'url dans data-src
    var videoUrl = $(this).data('src');
    if (!videoUrl) return; // sécurité
    var videoId = (videoUrl.split('?v=')[1] || '').trim();
    if (!videoId) return; // sécurité
    $('.cs-video_popup_container iframe').attr(
      'src',
